    ...
    <div class="grid lg:grid-cols-3 md:grid-cols-3 sm:grid-cols-1">
        <div class="flex justify-end items-center mb-[24px]">
            <a href="{{ route('admin.user.index') }}"
                class="flex items-center px-0 bg-transparent gap-1 text-heading-light text-paragraph hover:text-primary-400 transition-colors">
                User Lists
        ...
        <div class="w-full border border-base-500 p-5 rounded">
            <div class="grid lg:grid-cols-1 md:grid-cols-1 sm:grid-cols-1 sm:gap-1 md:gap-2">
                <div class="p-2 w-full">
                    ...
                    <x-forms.text-input type="text" wire:model.live='form.name' value="{{ $user->name }}"
            ...
            <div class="grid lg:grid-cols-1 md:grid-cols-1 sm:grid-cols-1 sm:gap-1 md:gap-2">
                <div class="p-2 w-full">
                    ...
                    <x-forms.text-input wire:model.live="form.email" value="{{ $user?->email }}" type="email"
            ...
            <div class="grid lg:grid-cols-1 md:grid-cols-1 sm:grid-cols-1 sm:gap-1 md:gap-2">
                <div class="p-2 w-full">
                    ...
                    <x-forms.select-input wire:model.live="form.department_id">
                        <option selected disabled>Department</option>
                        @forelse ($departments as $item)
                        <option value="{{ $item?->id }}" @selected(old('form.department_id', $item?->id
                            ??
            ...
            <div class="grid lg:grid-cols-1 md:grid-cols-1 sm:grid-cols-1 sm:gap-1 md:gap-2">
                <div class="p-2 w-full">
                    ...
                    <x-forms.select-input wire:model.live="form.role_id">
                        ...
                        <option value="{{ $role?->id }}" @selected(old('form.role_id', $user->roles->first()->id ?? '')
                            == $role->id)>
                            {{ ucfirst($role?->name) }}
                        </option>
                        @empty
